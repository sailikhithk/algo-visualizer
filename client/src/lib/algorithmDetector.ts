...
export type AlgorithmType =
  ...
  | "dp_edit_distance"
  | "dp_generic"
  | "two_pointers_generic"
  | "sliding_window_generic"
  | "backtracking_generic"
  | "nqueens"
  | "sudoku"
  | "permutations"
  | "combinations"
...
const PATTERNS: PatternMatch[] = [
  ...
  {
    pattern:
      /for\s+\w+\s+in\s+range\(.*\)[\s\S]*?for\s+\w+\s+in\s+range\(.*\)[\s\S]*?(>|<)[\s\S]*?(\bswap\b|\[\w+\]\s*,\s*\w+\[\w+\]\s*=\s*\w+\[\w+\]\s*,\s*\w+\[\w+\])/,
    type: "bubble_sort",
    category: "sorting",
    name: "Bubble Sort",
    timeComplexity: "O(n²)",
  ...
  {
    pattern:
      /\bmerge.?sort\b|def\s+merge\s*\(.*\)[\s\S]*?(left|right)[\s\S]*?def\s+\w*sort/i,
    type: "merge_sort",
    category: "sorting",
    name: "Merge Sort",
  ...
  },
  {
    ...
    description:
      "Picks a pivot, partitions array around it, then recursively sorts.",
    dataStructure: "array",
    priority: 10,
  },
  {
  ...
  {
    pattern:
      /\bbinary.?search\b|\bmid\s*=\s*\(?.*\+.*\)?\s*\/\/?\s*2|\blow\b.*\bhigh\b.*\bmid\b/i,
    type: "binary_search",
  ...
  {
    pattern:
      /\bbfs\b|\bbreadth.?first\b|from\s+collections\s+import\s+deque[\s\S]*?(visited|queue)/i,
    type: "bfs",
    category: "graph",
    ...
    spaceComplexity: "O(V)",
    description:
      "Explores all neighbors at present depth before moving to nodes at next depth level.",
    dataStructure: "graph",
    priority: 9,
  },
  {
  ...
  },
  {
    ...
    spaceComplexity: "O(V)",
    description:
      "Orders vertices in a DAG so every edge goes from earlier to later.",
    dataStructure: "graph",
    priority: 10,
  },
  ...
  {
    pattern:
      /class\s+\w*[Nn]ode[\s\S]*?\.left[\s\S]*?\.right|\.left\s*=|\.right\s*=/,
    type: "bst_insert",
    category: "tree",
    ...
    spaceComplexity: "O(n)",
    description:
      "Tree where left child is smaller and right child is larger than parent.",
  ...
  {
    pattern:
      /class\s+\w*[Nn]ode[\s\S]*?\.next[\s\S]*?reverse|reverse[\s\S]*?\.next/i,
    type: "linked_list_reverse",
  ...
  {
    pattern:
      /\bslow\b[\s\S]*?\bfast\b|\btortoise\b[\s\S]*?\bhare\b|\.next\.next/i,
  ...
  },
  {
    ...
    type: "linked_list_ops",
    category: "linked_list",
    name: "Linked List Operations",
    timeComplexity: "O(n)",
    spaceComplexity: "O(n)",
    description:
      "A linear collection of elements where each points to the next.",
    dataStructure: "linked_list",
    priority: 5,
  },
  ...
  {
    pattern:
      /\bstack\b[\s\S]*?(push|pop|append|\.pop\(\))|\bpush\b[\s\S]*?\bpop\b/i,
    type: "stack_ops",
  ...
  {
    pattern:
      /\bqueue\b[\s\S]*?(enqueue|dequeue|popleft)|from\s+collections\s+import\s+deque/i,
    type: "queue_ops",
    category: "stack_queue",
  ...
  },
  {
    ...
    spaceComplexity: "O(n)",
    description:
      "Solves complex problems by breaking into overlapping subproblems.",
    dataStructure: "array",
    priority: 4,
  },
  ...
  {
    pattern:
      /\btwo.?pointer\b|\bleft\b[\s\S]*?\bright\b[\s\S]*?while[\s\S]*?(left|right)\s*(\+|-)/i,
    type: "two_pointers_generic",
    category: "two_pointers",
    name: "Two Pointers",
    timeComplexity: "O(n)",
    spaceComplexity: "O(1)",
    description:
      "Uses two pointers that move toward each other or in the same direction.",
    dataStructure: "array",
    priority: 7,
  },
  ...
  {
    pattern:
      /\bsliding.?window\b|\bwindow\b[\s\S]*?(expand|shrink|left|right)/i,
    type: "sliding_window_generic",
  ...
  // BACKTRACKING
  {
    ...
    spaceComplexity: "O(n²)",
    description:
      "Places N queens on an N×N chessboard so no two attack each other.",
    dataStructure: "matrix",
    priority: 10,
  },
  {
    ...
    spaceComplexity: "O(n²)",
    description:
      "Fills a 9x9 grid so each row, column, and 3x3 box has digits 1-9.",
    dataStructure: "matrix",
    priority: 10,
  },
  {
  ...
  },
  {
    ...
    type: "backtracking_generic",
    category: "backtracking",
    name: "Backtracking",
    timeComplexity: "Exponential",
    spaceComplexity: "O(n)",
    description:
      "Tries all possible solutions, abandoning paths that fail constraints.",
    dataStructure: "array",
    priority: 6,
  },
  ...
  {
    pattern:
      /\bheapq\b|\bheap\b[\s\S]*?(push|pop|insert|extract)|import\s+heapq/i,
    type: "heap_ops",
    category: "heap",
    ...
    spaceComplexity: "O(n)",
    description:
      "Tree-based structure where parent is always greater/smaller than children.",
    dataStructure: "heap",
    priority: 8,
  },
  {
  ...
  {
    pattern:
      /\bunion.?find\b|\bdisjoint.?set\b|\bfind\b[\s\S]*?\bunion\b[\s\S]*?parent/i,
    type: "union_find_ops",
    category: "union_find",
    ...
    spaceComplexity: "O(n)",
    description:
      "Tracks elements partitioned into disjoint sets with near-constant operations.",
  ...
  {
    pattern:
      /\bdict\b\s*\(|\{\}[\s\S]*?\[.*\]\s*=|\bhash.?map\b|\bhash.?table\b/i,
    type: "hash_map_ops",
    category: "hash_map",
    name: "Hash Map",
  ...
  // COMPLEX / AI-VISUALIZED ALGORITHMS (high-priority exact name matches)
  {
    pattern: /findMedianSortedArrays|median.*sorted.*arrays|find.*median.*two/i,
    type: "median_two_sorted",
    category: "divide_conquer",
    name: "Median of Two Sorted Arrays",
    timeComplexity: "O(log(m+n))",
    spaceComplexity: "O(1)",
    description:
      "Binary search on two sorted arrays to find the combined median without merging.",
    dataStructure: "array",
    priority: 20,
  },
  {
    pattern: /trap.*rain|rain.*trap|\btrapping\b/i,
    type: "trapping_rain_water",
    category: "two_pointers",
    name: "Trapping Rain Water",
    timeComplexity: "O(n)",
    spaceComplexity: "O(1)",
    description:
      "Calculates water trapped between bars using two-pointer technique.",
    dataStructure: "array",
    priority: 20,
  },
  {
    pattern: /longest.*palindrom|palindromic.*substring|\bmanacher/i,
    type: "longest_palindrome",
    category: "dynamic_programming",
  ...
  {
    pattern: /\bwordBreak\b|word.*break/i,
    type: "word_break",
    category: "dynamic_programming",
    name: "Word Break",
    timeComplexity: "O(n²)",
  ...
  {
    pattern: /\bisMatch\b.*regex|regular.*expression.*match/i,
    type: "regex_match",
    category: "dynamic_programming",
    ...
    description:
      "Implements regex matching with '.'  and '*' using dynamic programming.",
    dataStructure: "matrix",
    priority: 20,
  },
  {
    pattern: /\bwildcard\b.*match/i,
    type: "wildcard_match",
    category: "dynamic_programming",
    ...
    spaceComplexity: "O(m·n)",
    description:
      "Wildcard pattern matching with '?' and '*' using dynamic programming.",
    dataStructure: "matrix",
    priority: 20,
  },
  {
    pattern: /next.*permut|\bnextPermutation\b/i,
    type: "next_permutation",
    category: "two_pointers",
  ...
  {
    pattern: /rotate.*image|\brotate\b.*matrix.*90/i,
    type: "rotate_image",
    category: "divide_conquer",
    name: "Rotate Image",
    timeComplexity: "O(n²)",
    spaceComplexity: "O(1)",
    description:
      "Rotates an n×n matrix 90 degrees in-place using transpose and reverse.",
    dataStructure: "matrix",
    priority: 20,
  },
...
export function detectAlgorithm(code: string): DetectedAlgorithm {
  ...
  return {
    ...
    spaceComplexity: "N/A",
    description:
      "Could not detect the algorithm. Try using recognizable function names.",
...
export function extractGraphFromCode(code: string): {
  nodes: string[];
...
export function extractTreeFromCode(code: string): {
  value: number;
  left?: any;
  right?: any;
} {
